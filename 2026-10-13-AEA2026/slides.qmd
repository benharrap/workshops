...
:::: {.columns}
::: {.column width=45%}
-   Important contextual knowledge (XXmin)
    -   Brief overview of R
    -   RStudio
    -   Packages
    -   R basics
...
## Folders and the working directory
...
Comments are just annotations you can add to code to help explain context, intention, functionality etc.

Bookmarks are denoted using 4, 5, or 6 hashes on either side of the bookmark title, and appear in the outline

::: {.callout-tip}
Get in the habit of writing comments, for your future self, and for anyone else!
:::
...
- `make` and `price` are arguments of the function
...
Pipes (`|>`) are used to chain functions together
...
'Under the hood', each factor level is a numeric value

This was our original character vector:

```{r}
animals
```

The numeric values depend on the level specification (which defaults to alphabetic)

::: {.panel-tabset}
### Unspecified levels

```{r}
levels(animals_unspecified)
as.integer(animals_unspecified)
```

### Specified levels
```{r}
levels(animals_specified)
as.integer(animals_specified)
```
:::
